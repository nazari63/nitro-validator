...
// adapted from https://github.com/dl-solarity/solidity-lib/blob/ceb1e347c56c335a7e680e7c224cf569cc2b37d4/contracts/libs/crypto/ECDSA384.sol
...
library ECDSA384 {
     ...
     *
     * Note that signatures from either the lower or upper part of the curve are accepted.
     */
    function verify(
        ...
        unchecked {
            ...
            /// allow compatibility with non-384-bit hash functions.
            {
                ...
                if (hashedMessageLength_ < 48) {
                    ...
                    unsafeCopy(
                        getDataPointer(hashedMessage_),
                        getDataPointer(tmp_) + 48 - hashedMessageLength_,
...

            {
                ...
                (scalar1,, scalar2) =
                    _doubleScalarMultiplication(call, params_.p, three, params_.a, points_, scalar1, scalar2);
            }
     ...
     */
    function _isOnCurve(uint256 call, uint256 p, uint256 a, uint256 b, uint256 x, uint256 y)
        private
        view
        returns (bool)
    {
    ...
    function _doubleScalarMultiplication(
        ...
        unchecked {
            ...
            for (uint256 word = 2; word <= 184; word += 2) {
                ...
                mask_ = (((scalar1Bits_ >> (184 - word)) & 0x03) << 2) | ((scalar2Bits_ >> (184 - word)) & 0x03);

                if (mask_ != 0) {
                    uint256[3] memory maskedPoints_ = points[mask_];

                    (x, y, z) =
                        _addProj(call, p, three, a, maskedPoints_[0], maskedPoints_[1], maskedPoints_[2], x, y, z);
                }
            }
            ...
            for (uint256 word = 2; word <= 256; word += 2) {
                ...
                mask_ = (((scalar1Bits_ >> (256 - word)) & 0x03) << 2) | ((scalar2Bits_ >> (256 - word)) & 0x03);
     ...
     */
    function _twiceProj(uint256 call, uint256 p, uint256 three, uint256 a, uint256 x0, uint256 y0, uint256 z0)
        private
        view
     ...
     */
    function _addProj2(uint256 call, uint256 v, uint256 u0, uint256 p, uint256 u1, uint256 t1, uint256 t0)
        private
        view
    ...
    function _precomputePointsTable(
        ...
        /// 0b0100: 1G + 0H
        (points_[0x04][0], points_[0x04][1], points_[0x04][2]) = (gx.copy(), gy.copy(), U384.init(1));
        /// 0b1000: 2G + 0H
        (points_[0x08][0], points_[0x08][1], points_[0x08][2]) =
            _twiceProj(call, p, three, a, points_[0x04][0], points_[0x04][1], points_[0x04][2]);
        /// 0b1100: 3G + 0H
        ...
        /// 0b0001: 0G + 1H
        (points_[0x01][0], points_[0x01][1], points_[0x01][2]) = (hx.copy(), hy.copy(), U384.init(1));
        /// 0b0010: 0G + 2H
        (points_[0x02][0], points_[0x02][1], points_[0x02][2]) =
            _twiceProj(call, p, three, a, points_[0x01][0], points_[0x01][1], points_[0x01][2]);
        /// 0b0011: 0G + 3H
     ...
     */
    function unsafeCopy(uint256 sourcePointer_, uint256 destinationPointer_, uint256 size_) private view {
        assembly {
     ...
     */
    function getDataPointer(bytes memory data_) private pure returns (uint256 pointer_) {
        assembly {
...
library U384 {
    ...
    function modexp(uint256 call_, uint256 b_, uint256 eInteger_) internal view returns (uint256 r_) {
        unchecked {
    ...
    function modexpAssignTo(uint256 call_, uint256 to_, uint256 b_, uint256 eInteger_) internal view {
        assembly {
            call_ := add(call_, EXP_OFFSET)
